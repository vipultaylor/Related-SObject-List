...
export default class SobjectList extends NavigationMixin(LightningElement) {
  ...
  renderedCallback() {
    if (!this.hasRendered) {
      let containerElement = this.template.querySelector(
        ".custom-sobject-list-container"
      );
      if (containerElement) {
  ...
  get breadcrumbSobjectURL() {
    return (
      "/lightning/r/" +
      this.responseWrapper.parentSObj.name +
      "/" +
      this.recordId +
      "/view"
    );
  ...
  get modalHeader() {
    let modalHeader;
    if (this.isNewModal) {
      modalHeader = this._formatLabel(
        this.LABEL.New_SObject,
        this.responseWrapper.sObj.label
      );
    } else if (this.isDeleteModal) {
      modalHeader = this._formatLabel(
        this.LABEL.Delete_SObject,
        this.responseWrapper.sObj.label
      );
    }
  ...
  createRecord() {
    if (
      this.responseWrapper.sObj.recordtypesEnabled &&
  ...
  handleDelete() {
    ...
    deleteRecord(deleteId)
      .then(() => {
        this.dispatchEvent(
          new ShowToastEvent({
            message: this._formatLabel(
              this.LABEL.Delete_Success_Message,
              this.responseWrapper.sObj.label,
              recordName
            ),
            variant: "success"
  ...
  _getDataFromApex() {
    ...
    if (
      this.recordId &&
      this.sObjectName &&
      this.commaSeparatedRecordtypes &&
      this.fieldSetForColumns &&
      this.idField &&
      ...
      getResponse({
        ...
        .then((result) => {
          ...
          this.responseWrapper.sObj.recordtypesEnabled =
            result.sObj.recordtypesEnabled;
          this.responseWrapper.sObj.availableRecordtypes =
              ...
              if (record[field.relationshipName]) {
                ...
                if (field.referenceTo) {
                  field.referenceTo.forEach((referenceObjName) => {
                    ...
                    if (
                      lstObjectsNotNavigatable.indexOf(referenceObjName) < 0
                    ) {
