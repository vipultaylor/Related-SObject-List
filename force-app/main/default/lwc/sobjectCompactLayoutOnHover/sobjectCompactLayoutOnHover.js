  ...
  wiredRecord({ error, data }) {
    if (error) {
      ...
      fieldsAsLayoutRows.forEach((element, index) => {
        ...
        let fieldDescribe = JSON.parse(
          JSON.stringify(
            data.objectInfos[this.objectApiName].fields[fieldApiName]
          )
        );
        ...
        if (fieldDescribe.dataType === "Reference") {
          fieldValue =
            data.records[this.recordId].fields[fieldDescribe.relationshipName]
              .displayValue;
        }
  ...
  renderedCallback() {
    ...
    if (sectionElement) {
      if (this.containerBoundingBoxLeft && this.containerBoundingBoxRight) {
        let popoverXPos =
          this.containerBoundingBoxRight -
          this.containerBoundingBoxLeft +
          NUBBIN_PADDING;
        let popoverYPos = -25;
    ...
    let titleIconElement = this.template.querySelector(
      ".custom-popover-title-icon"
    );
    if (titleIconElement) {
      titleIconElement.style["background-color"] =
        "#" + this.primaryObjectInfo.themeInfo.color;
    }
